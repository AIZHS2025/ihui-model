...
router = APIRouter(prefix="/cozeZhsApi/dashscope", tags=["通义千问视频合成"])
...
# --- WebSocket接口 ---
@router.websocket("/video-synthesis/ws")
async def video_synthesis_websocket(
    websocket: WebSocket,
    ...
    # 生成客户端ID（如果未提供）
    if not client_id:
        client_id = f"video_synthesis_{uuid.uuid4().hex[:8]}"

    logger.info(f"收到通义千问视频合成WebSocket连接请求: {client_id}")
    ...
    try:
        ...
        while True:
            try:
                ...
                # 视频合成请求
                if message_type == "video_synthesis":
                    synthesis_data = message.get("data", {})
                    prompt = synthesis_data.get("prompt", "")
                    chat_id = synthesis_data.get("chat_id", "")
                    ...
                    await manager.send_message(client_id, {
                        ...
                        "msg": "success",
                        "data": {
                            "type": "video_synthesis_started",
                            "content": "开始视频合成",
                            "chat_id": chat_id
                        },
                        "event": "video_synthesis.started"
                    })

                    # 模拟流式响应（简化版本）
                    # 在实际项目中，这里会调用通义千问视频合成API
                    video_url = f"https://example.com/video_synthesis_{uuid.uuid4().hex[:8]}.mp4"
                    ...
                    await manager.send_message(client_id, {
                        ...
                        "event": "video_synthesis.progress"
                    })
                    ...
                    await manager.send_message(client_id, {
                        ...
                        "event": "video_synthesis.completed"
                    })
