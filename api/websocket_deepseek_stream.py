...
router = APIRouter(prefix="/cozeZhsApi/ws", tags=["WebSocket DeepSeek流式"])
...
# --- WebSocket接口 ---
@router.websocket("/chatdeepseek/stream")
async def websocket_deepseek_stream(
    websocket: WebSocket,
    ...
    # 生成客户端ID（如果未提供）
    if not client_id:
        client_id = f"deepseek_{uuid.uuid4().hex[:8]}"

    logger.info(f"收到DeepSeek WebSocket流式连接请求: {client_id}")
        ...
        while True:
            try:
                ...
                if message_type == "chat":
                    ...
                    response_text = f"这是对 '{prompt}' 的DeepSeek流式响应。"
