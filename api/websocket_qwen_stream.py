...
router = APIRouter(prefix="/cozeZhsApi/ws/qwen", tags=["WebSocket通义千问流式"])
...
async def websocket_qwen_stream(
    websocket: WebSocket,
    ...
    logger.info(f"收到通义千问WebSocket流式连接请求: {client_id}")
        ...
        while True:
            try:
                ...
                if message_type == "chat":
                    ...
                    # 模拟流式响应（简化版本）
                    # 在实际项目中，这里会调用通义千问API
                    response_text = f"这是对 '{prompt}' 的流式响应。"
