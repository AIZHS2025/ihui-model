...
router = APIRouter(prefix="/cozeZhsApi/ws", tags=["WebSocket 智谱流式"])
...
# --- WebSocket接口 ---
@router.websocket("/zhipu/stream")
async def websocket_zhipu_stream(
    websocket: WebSocket,
    ...
    # 生成客户端ID（如果未提供）
    if not client_id:
        client_id = f"zhipu_{uuid.uuid4().hex[:8]}"

    logger.info(f"收到智谱WebSocket流式连接请求: {client_id}")
        ...
        while True:
            try:
                ...
                if message_type == "chat":
                    ...
                    # 模拟流式响应（简化版本）
                    # 在实际项目中，这里会调用智谱API
                    response_text = f"这是对 '{prompt}' 的智谱流式响应。"
